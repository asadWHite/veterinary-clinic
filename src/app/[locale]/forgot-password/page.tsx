...
export async function generateMetadata({
  ...
  return { title: `${translator.t("auth.forgot")} — ${translator.t("meta.siteTitle")}`, robots: { index: false } };
}
    ...
    <div className="border-b border-line">
      <div className="mx-auto grid w-full max-w-[100rem] grid-cols-1 gap-12 px-5 py-16 sm:px-8 lg:grid-cols-2 lg:gap-24 lg:px-12 lg:py-24">
        <div className="flex flex-col gap-6">
          <span className="label-eyebrow">{t("auth.forgot")}</span>
          <h1 className="text-h1 font-normal tracking-tight text-ink">{t("auth.forgot")}</h1>
          <p className="max-w-md text-[0.98rem] leading-relaxed text-ink-2">{t("auth.forgotLead")}</p>
        </div>
