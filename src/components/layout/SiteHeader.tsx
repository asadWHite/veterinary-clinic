...
export function SiteHeader({
  ...
  // `openFor` holds the pathname the menu was opened on: navigating closes it,
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-line bg-canvas/95 backdrop-blur-[2px]">
      <div className="mx-auto flex h-[4.5rem] w-full max-w-[100rem] items-center justify-between gap-6 px-5 sm:px-8 lg:px-12">
        <Link
          href={`/${locale}`}
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <div className="relative hidden sm:block">
            ...
            {langOpen && (
              <div
                role="menu"
                className="absolute right-0 top-8 z-50 w-28 border border-line bg-canvas py-1 shadow-[0_18px_40px_-30px_rgba(37,37,34,0.5)]"
              >
                {LOCALES.map((code) => (
                  <button
                    key={code}
          ...
          >
            ...
            <span aria-hidden className="flex flex-col gap-[5px]">
              <span className={cn("block h-px w-5 bg-ink transition-transform", open && "translate-y-[6px] rotate-45")} />
      ...
      {open && (
        <div
          id="mobile-nav"
          className="fixed inset-x-0 top-[4.5rem] bottom-0 z-40 overflow-y-auto border-t border-line bg-canvas px-5 py-8 lg:hidden"
        ...
        >
          ...
          <div className="mt-8 flex flex-col gap-4">
            <Link href={`/${locale}/book`} className="btn btn-primary w-full">
              {t("nav.book")}
            </Link>
            {user ? (
              <>
                <Link href={`/${locale}/account`} className="btn btn-ghost w-full">
                  {t("nav.account")}
                </Link>
                {user.role !== "user" && (
                  <Link href={`/${locale}/admin`} className="btn btn-quiet w-full">
                    {t("nav.admin")}
                  </Link>
                )}
                <form action={logoutAction}>
              ...
              <Link href={`/${locale}/login`} className="btn btn-ghost w-full">
                {t("nav.login")}
              </Link>
            )}
          </div>
